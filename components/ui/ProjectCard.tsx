...
export default function ProjectCard({
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div
        className={`bg-almond/60 h-full border border-gold shadow-none hover:border-cartier/40 hover:shadow-[0_8px_40px_rgba(0,0,0,0.3)] transition-all duration-500 group rounded-none overflow-hidden flex flex-col cursor-pointer border-l-[3px] ${isBackend ? "border-l-coffee/50" : "border-l-[#6A89A7]/60"}`}
      ...
      >
        ...
        <div className="px-6 pt-5 pb-3">
          <div className="flex justify-between w-full items-start gap-2 mb-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0 mt-1">
              {isBackend && (
                <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 border border-gold text-[#6A89A7] rounded-full">
                  API
                </span>
              )}
              {url && (
                <ArrowUpRight
                  size={16}
                  className="text-[#384959] opacity-0 group-hover:opacity-100 group-hover:text-cartier transition-all duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5"
                />
              )}
            </div>
          </div>
        ...
        <div className="px-6 py-3 flex-1">
          ...
          <ul className="space-y-1.5">
            {features.slice(0, 2).map((feature, i) => (
              <li key={i} className="flex items-start gap-2.5 text-xs text-[#8AAFC4] leading-relaxed">
        ...
        <div className="px-6 py-4">
          ...
          <div className="space-y-2.5">
            {keyDecisions.map((decision, i) => (
              <p key={i} className="text-xs text-[#8AAFC4] leading-relaxed">
                {decision}
              </p>
            ))}
          </div>
        ...
        <div className="px-6 py-4 flex justify-between items-center">
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#6A89A7]">
            Source Code
          </span>
          <Button
            variant="outline"
