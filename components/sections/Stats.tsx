...
export default function Stats({ stats }: StatsProps) {
  return (
    <section aria-label="Key figures" className="w-full bg-[#111F35] border-y border-gold/20 py-12 px-6 md:px-12 lg:px-24">
      <motion.div
        ...
        {stats.map((stat, i) => (
          <div key={i} className="flex items-center flex-1">
            <div className="flex flex-col items-center gap-2 flex-1">
              ...
              <span className="text-[9px] uppercase tracking-[0.3em] text-[#6B7A8D] font-bold text-center">
                {stat.label}
              </span>
            </div>
            {i < stats.length - 1 && (
